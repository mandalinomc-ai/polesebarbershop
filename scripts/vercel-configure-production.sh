...
# Usage: VERCEL_TOKEN=xxx ./scripts/vercel-configure-production.sh [SITE_URL]
set -euo pipefail

cd "$(dirname "$0")/.."

: "${VERCEL_TOKEN:?Set VERCEL_TOKEN from https://vercel.com/account/tokens}"
VERCEL_ORG_ID="${VERCEL_ORG_ID:-team_mandalinomc-8144s-projects}"
...
if [[ -f .env.local ]]; then
  ...
  set -a
  # RESEND_FROM contains spaces — load known keys manually
