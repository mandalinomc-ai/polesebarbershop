...
describe("mobile QA (390px)", () => {
  ...
  it("keeps /gestionale usable on a phone (bottom nav + logout + walk-in taps)", () => {
    ...
    expect(crm).toMatch(/Blocco temporaneo/);
    expect(crm).toMatch(/\/api\/admin\/calendar-blocks/);
    expect(crm).toMatch(/Prenotazione recente/);
    expect(css).toMatch(/\.crm-mobile-logout \{ display: inline-flex; \}/);
    expect(css).toMatch(/\.occupancy-table td\.taken/);
    expect(css).toMatch(/\.occupancy-table td\.blocked/);
    expect(css).toMatch(/\.cal-day\.salon-closed/);
    expect(css).toMatch(/\.crm-recent-booking/);
  });
