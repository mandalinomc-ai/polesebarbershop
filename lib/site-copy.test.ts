...
describe("public copy vs official identity", () => {
  ...
  it("keeps shop WhatsApp on wa.me/393270156225 without Twilio", () => {
    ...
    expect(wizard).toMatch(/Riceverai conferma via email/);
    expect(wizard).toMatch(/non partiamo in automatico/);
    expect(wizard).toMatch(/customerWhatsAppSent/);
  ...
  it("defaults to live site with July 3 marble layout and booking", () => {
    ...
    expect(landing).toMatch(/TreatmentCards/);
    expect(landing).toMatch(/SfumatureSection/);
  ...
  it("keeps bio+Felice video, trattamenti cards, sfumature, listino and contact", () => {
    const landing = readFileSync(join(process.cwd(), "components/site/LandingSections.tsx"), "utf8");
    expect(landing).not.toMatch(/Consulenza Tricologica/i);
    expect(landing).toMatch(/ConsulenzaSection/);
    const consulenza = readFileSync(join(process.cwd(), "components/site/ConsulenzaSection.tsx"), "utf8");
    expect(consulenza).toMatch(/id="consulenza"/);
    expect(consulenza).toMatch(/Consulenza Gratuita/);
    expect(landing).toMatch(/id="about"/);
    ...
    const trattamentiIdx = landing.indexOf("<TreatmentCards");
    const consulenzaIdx = landing.indexOf("<ConsulenzaSection");
    const sfumatureIdx = landing.indexOf("<SfumatureSection");
    const prenotaIdx = landing.indexOf('id="prenota"');
    const contactIdx = landing.indexOf('id="contact"');
    expect(aboutIdx).toBeGreaterThan(-1);
    expect(trattamentiIdx).toBeGreaterThan(aboutIdx);
    expect(consulenzaIdx).toBeGreaterThan(trattamentiIdx);
    expect(sfumatureIdx).toBeGreaterThan(consulenzaIdx);
    expect(prenotaIdx).toBeGreaterThan(sfumatureIdx);
    ...
    const treatment = readFileSync(join(process.cwd(), "components/site/TreatmentCards.tsx"), "utf8");
    expect(treatment).toMatch(/id="trattamenti"/);
    expect(treatment).toMatch(/Menu/);
    expect(treatment).toMatch(/Servizi/);
    expect(treatment).toMatch(/trattamenti-grid/);
    expect(treatment).toMatch(/treatment-card-media/);
    expect(treatment).toMatch(/Prenota ora/);
    expect(treatment).toMatch(/formatCardPrice/);
    const sfumature = readFileSync(join(process.cwd(), "components/site/SfumatureSection.tsx"), "utf8");
    expect(sfumature).toMatch(/id="sfumature"/);
    expect(sfumature).toMatch(/CUTTING_TECHNIQUE_VIDEOS/);
    expect(sfumature).toMatch(/Taper Fade/);
    expect(sfumature).toMatch(/Burst Fade/);
    expect(sfumature).toMatch(/Razor Fade/);
    expect(sfumature).not.toMatch(/€/);
    expect(sfumature).not.toMatch(/Prenota ora/);
    const videos = readFileSync(join(process.cwd(), "lib/site-videos.ts"), "utf8");
    expect(videos).toMatch(/label: "Taper Fade"/);
    expect(videos).toMatch(/label: "Burst Fade"/);
    expect(videos).toMatch(/label: "Razor Fade"/);
    expect(videos).not.toMatch(/salone-generale/);
    expect(videos).not.toMatch(/id: "razor-taper"/);
    expect(videos).not.toMatch(/id: "skin-fade"/);
    const chrome = readFileSync(join(process.cwd(), "components/site/Chrome.tsx"), "utf8");
    expect(chrome).toMatch(/href: "\/#trattamenti", label: "Servizi"/);
    expect(chrome).toMatch(/href: "\/#sfumature", label: "Sfumature"/);
