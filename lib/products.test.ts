...
describe("Felice Polese product vetrina", () => {
  ...
  it("prepares WhatsApp info copy without checkout language", () => {
    expect(productOrderMessage("Cera Lucida")).toMatch(/informazioni su Cera Lucida/);
    expect(productOrderMessage("Cera Lucida")).toMatch(/prezzi e disponibilità/);
    expect(productOrderMessage("Cera Lucida")).not.toMatch(/acquista|checkout|carrello/i);
  });
});
